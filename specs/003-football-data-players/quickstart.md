...
- 2026-09-23 (T057, validación manual local con PostgreSQL 17, Memurai y Football-Data real): API key (`401` sin clave o con clave inválida, `200` con clave válida); filtros por liga, posición, combinados con equipo case-insensitive, sin coincidencias (`0`) y valor inválido (`400`); `X-Correlation-ID` propagado y generado; 2649 jugadores con 2649 asignaciones `100/100/1.00` al superusuario y sin `external_id` duplicados; auditoría con `UPDATE`/`DELETE` rechazados (`append-only`); claves por liga en Redis con TTL ≤ 6 h; `/actuator/health` `UP`, `/actuator/metrics` `401` sin clave y métricas `football.data.*`/`catalog.synchronization*` presentes; Swagger con filtros, `200/400/401/503`, `apiKeyAuth` y ejecución `200` en menos de 2 minutos. En una base vacía descartable: WARN `superuser_unavailable` al arrancar, `503 catalog_unavailable`, y tras promover el usuario a `ADMIN` la sincronización terminó `COMPLETED` en el siguiente reintento (`PT1M`) sin reiniciar, con `/players` en `200`.
