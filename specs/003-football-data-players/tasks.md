...
- [X] T060 [US1] Escribir tests (deben fallar primero) donde un jugador del lote falla al persistir entre jugadores válidos: los válidos quedan guardados con sus tokens, se registra exactamente un `PLAYER_FAILED` con externalId y failureCode sanitizado, y el resultado es `PARTIAL_FAILURE`; sin fallas el resultado sigue siendo `COMPLETED`; si fallan todos los jugadores intentados el resultado es `FAILED`, en `backend/src/test/java/com/example/demo/service/PlayerCatalogServiceTest.java` per US1/AC7, SC-002 (missing)
- [X] T061 [US1] Reemplazar la transacción única de `synchronizeCatalog()` por una transacción por jugador (upsert + asignación de 100 tokens + sus eventos de auditoría juntos, vía `TransactionTemplate` o un bean `@Transactional` separado; no usar `@Transactional` en un método llamado desde la misma clase porque el proxy no lo intercepta), manteniendo la validación del superusuario antes de cualquier write, en `backend/src/main/java/com/example/demo/service/PlayerCatalogService.java` per FR-023, FR-017 (missing)
- [X] T062 [US1] Ante la falla de un jugador: revertir solo ese jugador, anexar `PLAYER_FAILED` en transacción independiente, contar jugadores fallidos, continuar con el resto y derivar el estado terminal (`COMPLETED` sin fallas; `FAILED` si hubo alguna falla y ningún jugador se persistió, lo que incluye todas las ligas fallidas; `PARTIAL_FAILURE` en otro caso) incluyendo el conteo en el evento terminal y el log, en `backend/src/main/java/com/example/demo/service/PlayerCatalogService.java` per FR-023, SC-002 (missing)
- [X] T063 [US1] Escribir tests (deben fallar primero) del reintento de arranque: mientras no existe snapshot exitoso cada tick ejecuta la sincronización; tras el primer éxito el tick no hace nada; un intervalo menor a 1 minuto se rechaza; el lock evita solapamiento con el cron, en `backend/src/test/java/com/example/demo/scheduler/PlayerCatalogSchedulerTest.java` per US1/AC8, FR-016 (missing)
- [X] T064 Agregar `bootstrapRetryInterval` (Duration, default `PT5M`, validado ≥ 1 minuto) a las propiedades tipadas, mapeado como `football-data.bootstrap-retry-interval=${FOOTBALL_DATA_BOOTSTRAP_RETRY_INTERVAL:PT5M}`, en `backend/src/main/java/com/example/demo/config/FootballDataProperties.java`, `backend/src/main/resources/application.properties` y `backend/src/test/resources/application.properties` per FR-016 (missing)
- [X] T065 [US1] Implementar el reintento de arranque con fixed delay igual a `bootstrapRetryInterval` que ejecute `runOnce()` solo mientras `hasSuccessfulSnapshot()` sea falso, compartiendo el lock existente, en `backend/src/main/java/com/example/demo/scheduler/PlayerCatalogScheduler.java` per FR-016, US1/AC8 (missing)
- [X] T066 [US1] Loguear WARN en cada intento fallido con el código de causa sanitizado; para `SuperuserUnavailableException` indicar que `MARKET_SUPERUSER_USERNAME` debe referenciar un usuario `ADMIN` existente, sin auditar (no hay actor válido), en `backend/src/main/java/com/example/demo/scheduler/PlayerCatalogScheduler.java` per FR-016, FR-015 (partial)
- [X] T067 [US1] Hacer que `PLAYER_CREATED`, `PLAYER_UPDATED` y `TOKENS_ALLOCATED` se anexen dentro de la transacción del jugador (p. ej. método con `Propagation.MANDATORY`), dejando `REQUIRES_NEW` solo para `STARTED`, eventos terminales y `PLAYER_FAILED`, para que un jugador revertido no deje auditoría huérfana, en `backend/src/main/java/com/example/demo/service/CatalogSyncAuditService.java`, `backend/src/main/java/com/example/demo/service/PlayerTokenInitializationService.java` y `backend/src/main/java/com/example/demo/service/PlayerCatalogService.java` per FR-023 (contradicts)
- [X] T068 [US1] Escribir test que simule un error interno no asociado a un jugador después de `STARTED` y verifique que se anexa `FAILED` con `internal_persistence_error` y la excepción se propaga, en `backend/src/test/java/com/example/demo/service/PlayerCatalogServiceTest.java` per FR-019 (partial)
- [X] T069 Extender la integración PostgreSQL con un esquema legacy baselined que tenga `players.position` NOT NULL y verificar que tras V3 la columna no existe y un jugador se inserta con posiciones en `player_positions`, en `backend/src/test/java/com/example/demo/repository/PlayerCatalogPersistenceIntegrationTest.java` per FR-004, plan: baseline Flyway (partial)
- [X] T070 [P] Documentar `FOOTBALL_DATA_BOOTSTRAP_RETRY_INTERVAL`, el flujo de primer arranque sin superusuario y el evento `PLAYER_FAILED` en `README.md` y `specs/003-football-data-players/quickstart.md` per FR-016, FR-023 (missing)
